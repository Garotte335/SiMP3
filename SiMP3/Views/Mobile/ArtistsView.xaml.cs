...
public partial class ArtistsView : ContentView
{
    ...
    private async Task PromptArtistAsync()
    {
        var artists = _controller.GetAllTracksSnapshot()
            .Select(t => t.Artist)
        ...
        var choice = await Application.Current!.MainPage.DisplayActionSheet("Обрати виконавця", "Скасувати", null, artists);
    ...
    {
        if (!string.IsNullOrWhiteSpace(_selectedArtist))
            _controller.SetArtistFilter(_selectedArtist);
